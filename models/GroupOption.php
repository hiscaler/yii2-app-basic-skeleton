...
class GroupOption extends BaseActiveRecord
{
    ...
    {
        $names = [];
        $rawData = (new Query)->select(['group_name'])->distinct(true)->from([static::tableName()])->where('tenant_id = :tenantId', [':tenantId' => Yad::getTenantId()])->column();
    ...
    {
        $options = [];
        $rawData = Yii::$app->db->createCommand("SELECT [[group_name]], [[text]], [[value]], [[enabled]] FROM {{%group_option}} ORDER BY [[group_name]], [[ordering]]")->queryAll();
        foreach ($rawData as $data) {
    ...
    public static function getText($groupName, $value)
    {
        ...
        if (!empty($groupName) && !empty($value)) {
            $text = Yii::$app->db->createCommand('SELECT [[text]] FROM {{%group_option}} WHERE [[tenant_id]] = :tenantId AND [[group_name]] = :groupName AND [[value]] = :value')->bindValues([
